...
def load_gt_seg_from_json(json_file, split=None, label='label_id', label_offset=0):
    ...
    vids, starts, stops, labels, times = [], [], [], [], []
    for k, v in json_db.items():
        ...
        for event in ants:
            ...
            # times += [(float(event['segment'][0]) + float(event['segment'][1]))/2]
            times += [(float(event['segment']))]
...
class ANETdetection(object):
    ...
    def wrapper_compute_average_precision(self, preds):
        ...
        #     delayed(compute_average_precision_distance)(
        ...
        results = Parallel(n_jobs=self.num_workers)(
            delayed(compute_average_precision_distance)(
                ground_truth=ground_truth_by_label.get_group(cidx).reset_index(drop=True),
                prediction=self._get_predictions_with_label(prediction_by_label, label_name, cidx),
                dist_thresholds=self.tiou_thresholds,
            ) for label_name, cidx in self.activity_index.items())
    ...
    def evaluate_partial(self, preds, verbose=True):
        """Evaluates a prediction file. For the detection task we measure the
        interpolated mean average precision to measure the performance of a
        method.
        preds can be (1) a pd.DataFrame; or (2) a json file where the data will be loaded;
...
def compute_average_precision_distance(
    ground_truth,
    prediction,
    dist_thresholds=np.linspace(0.1, 0.5, 5)
):
    """Compute average precision (detection task) between ground truth and
    predictions data frames. If multiple predictions occurs for the same
    predicted segment, only the one with highest score is matches as
    ...
    prediction : df
        Data frame containing the prediction instances.
        Required fields: ['video-id, 't-start', 't-end', 'score']
    dist_thresholds : 1darray, optional
        Temporal intersection over union threshold.
    Outputs
    -------
    ap : float
        Average precision score.
    """
    ap = np.zeros(len(dist_thresholds))
    if prediction.empty:
        return ap

    npos = float(len(ground_truth))
    lock_gt = np.ones((len(dist_thresholds), len(ground_truth))) * -1
    # Sort predictions by decreasing score order.
    sort_idx = prediction['score'].values.argsort()[::-1]
    prediction = prediction.loc[sort_idx].reset_index(drop=True)

    # Initialize true positive and false positive vectors.
    tp = np.zeros((len(dist_thresholds), len(prediction)))
    fp = np.zeros((len(dist_thresholds), len(prediction)))
    ...
    for idx, this_pred in prediction.iterrows():
        ...
        this_gt = ground_truth_videoid.reset_index()
        dist_arr = distance_to_gt(this_pred[['times']].values,
                               this_gt[['times']].values.flatten())
        # We would like to retrieve the predictions with highest tiou score.
        dist_sorted_idx = dist_arr.argsort()
        for tidx, dist_thr in enumerate(dist_thresholds):
            for jdx in dist_sorted_idx:
                if dist_arr[jdx] > dist_thr:
                    fp[tidx, idx] = 1
                    break
                if lock_gt[tidx, this_gt.loc[jdx]['index']] >= 0:
                    continue
    ...
    for tidx in range(len(dist_thresholds)):
        ap[tidx] = interpolated_prec_rec(precision_cumsum[tidx,:], recall_cumsum[tidx,:])
...
def distance_to_gt(target_timestamp, candidate_timestamps):
    """Compute the temporal intersection over union between a
    target segment and all the test segments.
    Parameters
    ----------
    target_timestamp : 1d array
        Temporal target segment containing [starting, ending] times.
    candidate_timestamps : 2d array
        Temporal candidate segments containing N x [starting, ending] times.
    Outputs
    -------
    tiou : 1d array
    ...
    return np.abs(candidate_timestamps - target_timestamp)
...
if __name__ == '__main__':
    ground_truth = pd.DataFrame(
        {
            'video-id': ['vid1', 'vid2', 'vid3', 'vid1', 'vid2', 'vid3', 'vid4', 'vid2'],
            'time': [3, 20, 95, 48, 22, 33, 55, 88],
        }
    )
    ...
    pred1 = pd.DataFrame(
        {
            'video-id': ['vid1', 'vid1', 'vid2', 'vid3', 'vid1'],
            'time': [3, 3, 20, 95, 48],
            'score': [0.2, 0.3, 1.0, 1.0, 0.8],
        }
    )
    pred2 = pd.DataFrame(
        {
            'video-id': ['vid2', 'vid3', 'vid4', 'vid2'],
            'time': [22, 33, 55, 88],
        }
    )
    out = compute_average_precision_distance(ground_truth, pred1, dist_thresholds=[5, 7])
    print(out)
