...
def train_one_epoch(
    ...
    for iter_idx, video_list in enumerate(train_loader, 0):
        ...
        loss_weight = losses['loss_weight'].item()
    ...
    for iter_idx, video_list in enumerate(val_loader, 0):
        ...
        with torch.no_grad():
            ...
            for vid_idx in range(num_vids):
                if output[vid_idx]['scores'].shape[0] > 0:
                    results['video-id'].extend(
                        [output[vid_idx]['video_id']] *
                        output[vid_idx]['scores'].shape[0]
                    )
                    results['times'].append(output[vid_idx]['times'].squeeze())
                    # results['t-end'].append(output[vid_idx]['segments'][:, 1])
                    results['label'].append(output[vid_idx]['labels'].squeeze())
                    results['score'].append(output[vid_idx]['scores'].squeeze())
