...
Tensor softnms_1d_cpu(Tensor scores, Tensor dets, float iou_threshold,
                      float sigma, float min_score, int method) {
  if (scores.numel() == 0) {
    return at::empty({0}, scores.options().dtype(at::kLong));
  }

  auto scores_t = scores.clone();

  auto nsegs = scores.size(0);
  auto sc = scores_t.data_ptr<float>();
  auto de = dets.data_ptr<float>();

  int64_t pos = 0;
  Tensor inds_t = at::arange(nsegs, scores.options().dtype(at::kLong));
...
//      auto ovr = inter / (iarea + areas[pos] - inter);
      auto dist = std::abs(inds[pos] - iind);
      float weight = 1.;
      if (method == 0) {
        // vanilla nms
        if (dist <= iou_threshold) weight = 0;
      } else if (method == 1) {
        // linear
        if (dist <= iou_threshold) weight = 1 - dist;
      } else if (method == 2) {
        // gaussian
        weight = 1 - std::exp(-(dist * dist) / sigma);
      }
      sc[pos] *= weight;
...
Tensor softnms_1d(Tensor scores, Tensor dets, float iou_threshold,
                  float sigma, float min_score, int method) {
  // softnms is not implemented on GPU
  CHECK_CPU_INPUT(scores)
  CHECK_CPU_INPUT(dets)
  return softnms_1d_cpu(scores, dets, iou_threshold, sigma, min_score, method);
}
