...
class SoftNMSop(torch.autograd.Function):
    ...
    def forward(
        ...
        dets = scores.new_empty((scores.size(0), 1), device='cpu')
        ...
        if max_num > 0:
            ...
            n_segs = len(inds)
        # print("-------------------------------------------------------------")
        # print("Nsegs:", type(n_segs))
        # print("-------------------------------------------------------------")
        # if type(n_segs) != int:
        #     breakpoint()
        # import sys
        # sys.exit()
        sorted_scores = dets[:n_segs]
        sorted_cls_idxs = cls_idxs[inds]
...
def batched_nms(
    ...
    if multiclass:
        ...
        for class_id in torch.unique(cls_idxs):
            ...
            if use_soft_nms:
                ...
                sorted_scores, sorted_cls_idxs = NMSop.apply(
                    segs[curr_indices],
                    scores[curr_indices],
                    cls_idxs[curr_indices],
                    iou_threshold,
    ...
    _, idxs = new_scores.view(-1).sort(descending=True)
    max_seg_num = min(max_seg_num, new_scores.shape[0])
    # print("Max seg num", max_seg_num, type(max_seg_num))
    # print("Inds", type(idxs))
...
if __name__ == '__main__':
    ...
    scores = torch.as_tensor(
        [0.5, 0.2, 0.02, 0.3, 0.8, 0.35, 0.23, 0.85, 0.332, 0.1, 0.2, 0.1, 0.9]
    )
    dets = scores.new_empty((scores.size(0), 1), device='cpu')
    ...
    print(inds)
    opp_scores = 1 - scores
    all_scores = torch.cat((scores.unsqueeze(0), opp_scores.unsqueeze(0)))
    lists = all_scores.cpu().numpy().tolist()
    print(lists)
    for l in lists:
        for a in l:
            print(f'{a:.2f} & ', end='')
